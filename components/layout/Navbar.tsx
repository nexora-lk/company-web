...
export default function Navbar() {
    ...
    return (
        <header className="fixed w-full top-0 z-50 bg-bg border-b border-line lg:border-0 lg:bg-transparent lg:px-6 xl:px-8 lg:pt-4 xl:pt-5">
            <div className="max-w-content mx-auto px-6 md:px-8 lg:px-5 xl:px-6 h-16 lg:h-16 xl:h-18 flex items-center justify-between gap-4 xl:gap-6 lg:rounded-full lg:bg-bg lg:border lg:border-line">
                {/* ── Logo ── */}
                <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group shrink-0">
                    ...
                    <span className="hidden sm:flex flex-col leading-none">
                        <span className="font-semibold text-[14px] sm:text-[15px] lg:text-[17px] xl:text-[18px] tracking-tightish whitespace-nowrap transition-colors duration-300 group-hover:text-accent">
                            {brand.name}
                        </span>
                            ...
                            return (
                                <div key={link.label} className="relative group">
                                    <button
                                        ...
                                        className={`px-4 xl:px-4.5 h-[36px] xl:h-[40px] rounded-full text-[13.5px] xl:text-[14px] font-medium transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer border-none ${
                                            isParentActive
                                                ...
                                                : 'text-sapphire-blue/80 hover:text-sapphire-blue hover:bg-sapphire-blue/5'
                                        }`}
                                    >
                                        {link.label}
                                        <ChevronDown
                                    ...
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 translate-y-1 pointer-events-none transition-all duration-200 group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
                                        <div className="bg-bg border border-none rounded-2xl shadow-lg p-1.5 w-48 xl:w-52 flex flex-col gap-0.5">
                                                ...
                                                return (
                                                    <Link
                                                        ...
                                                        className={`px-4 py-2.5 rounded-full text-[13px] xl:text-[13.5px] font-medium transition-colors ${
                                                            childActive
                                                                ? 'bg-sapphire-blue/10 text-sapphire-blue'
                                                                : 'text-sapphire-blue/70 hover:text-sapphire-blue hover:bg-surface'
                                                        }`}
                                                    >
                                                        {child.label}
                                                    </Link>
                        ...
                        return (
                            <Link
                                ...
                                className={`px-4 xl:px-4.5 h-[36px] xl:h-[40px] flex items-center justify-center rounded-full text-[13.5px] xl:text-[14px] font-medium transition-all duration-200 ${
                                    isActive
                                        ? 'bg-sapphire-blue/10 text-sapphire-blue'
                                        : 'text-ink/80 hover:text-sapphire-blue hover:bg-sapphire-blue/5'
                                }`}
                                href={link.href}
                            >
                                {link.label}
                ...
                <Link
                    href="/#contact"
                    className="hidden lg:inline-flex items-center justify-center gap-3 whitespace-nowrap btn bg-sapphire-blue text-bg hover:-translate-y-px hover:bg-normal-gold hover:text-black !min-h-0 h-[40px] xl:h-[46px] text-[13px] xl:text-[13.5px] font-semibold px-5 xl:px-6 rounded-full transition-all duration-200 hover:gap-4 group"
                >
